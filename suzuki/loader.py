...
from suzukitl.tl.tlobject import TLObject
...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _suzuki_client_id_logging_tag = copy.copy(  # noqa: F841
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?suzuki$", from_id=0000000000)
...
class Modules:
    ...
    def __init__(
        ...
        self.client.suzuki_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.suzuki_commands)
                inline_handlers.update(module.suzuki_inline_handlers)
                callback_handlers.update(module.suzuki_callback_handlers)
                watchers.extend(module.suzuki_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.suzuki_commands))
            )

        for _command, cmd in instance.suzuki_commands.items():
        ...
        for alias, cmd in self.aliases.copy().items():
            if cmd in instance.suzuki_commands:
                self.add_alias(alias, cmd)

        self.register_inline_stuff(instance)

    def register_inline_stuff(self, instance: Module):
        for name, func in instance.suzuki_inline_handlers.copy().items():
        ...
        for name, func in instance.suzuki_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.suzuki_watchers.values():
    ...
    def get_prefix(self, userbot: typing.Optional[str] = None) -> str:
        """Get prefix for specific userbot. Pass `None` to get Suzuki prefix"""
        if userbot == "dragon":
            key = "dragon.prefix"
            default = ","
        else:
