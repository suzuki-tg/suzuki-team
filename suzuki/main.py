...
#    Suzuki Userbot
#    Copyright (C) 2023-2024 The Authors
...
import suzuki_tl
from suzuki_tl import events
from suzuki_tl.errors import (
    ApiIdInvalidError,
...
from suzuki_tl.network.connection import (
    ConnectionTcpFull,
    ConnectionTcpMTProxyRandomizedIntermediate,
)
from suzuki_tl.password import compute_check
from suzuki_tl.sessions import MemorySession, SQLiteSession
from suzuki_tl.tl.functions.account import GetPasswordRequest
from suzuki_tl.tl.functions.auth import CheckPasswordRequest
...
# fmt: off
JAPANESE_NAMES = [
    "Sakura", "Haruka", "Aoi", "Yuki", "Hikari", "Akari",
    "Kaito", "Ren", "Sora", "Riku", "Haru", "Natsumi",
    "Yume", "Hana", "Rin", "Asuka", "Mei", "Koharu",
    "Suzuki", "Tanaka", "Yamamoto", "Nakamura", "Kobayashi",
    "Yoshida", "Yamada", "Sasaki", "Matsumoto", "Inoue",
    "Kimura", "Hayashi", "Shimizu", "Yamaguchi", "Mori",
    "Hashimoto", "Yamashita", "Ishikawa", "Nakajima", "Maeda",
    "Fujita", "Ogawa", "Goto", "Okada", "Hasegawa", "Murakami",
    "Kondo", "Ishii", "Saito", "Sakamoto", "Endo", "Aoki",
    "Fujii", "Nishimura", "Fukuda", "Ota", "Miura", "Fujiwara",
    "Okamoto", "Matsuda", "Nakagawa", "Nakano", "Harada", "Ono",
    "Tamura", "Takeuchi", "Kaneko", "Wada", "Nakayama", "Ishida",
    "Ueda", "Morita", "Hara", "Shibata", "Sakai", "Kudo",
    "Yokoyama", "Miyazaki", "Miyamoto", "Uchida", "Takagi",
    "Ando", "Taniguchi", "Ohno", "Maruyama", "Imai", "Takada",
    "Fujimoto", "Takeda", "Murata", "Ueno", "Sugiyama", "Masuda",
    "Nakatsuka", "Noguchi", "Kato", "Otsuka", "Kikuchi", "Shimada"
]
# fmt: on
...
def generate_app_name() -> str:
    ...
    :example: "Suzuki Sakura Haruka"
    """
    return " ".join(random.choices(JAPANESE_NAMES, k=2))
...
class Suzuki:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("suzuki-") and f.endswith(".session"),
                os.listdir(BASE_DIR),
            )
        ]
    ...
    async def save_client_session(self, client: CustomTelegramClient):
        ...
        client.suzuki_db = database.Database(client)
        await client.suzuki_db.init()
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            await client.suzuki_inline.bot.send_animation(
                logging.getLogger().handlers[0].get_logid_by_client(client.tg_id),
                "https://github.com/suzuki-team/assets/raw/master/suzuki_banner.mp4",
                caption=(
                    "🚗 <b>Suzuki {} started!</b>\n\n🌳 <b>GitHub commit SHA: <a"
                    ' href="https://github.com/suzuki-team/Suzuki/commit/{}">{}</a></b>\n✊'
                    " <b>Update status: {}</b>\n<b>{}</b>".format(
                        ".".join(list(map(str, list(__version__)))),
                        build,
                        build[:7],
            ...
            logging.debug(
                ...
                client.tg_id,
                client.suzuki_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
...
suzuki_tl.extensions.html.CUSTOM_EMOJIS = not get_config_key("disable_custom_emojis")
