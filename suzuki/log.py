...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of Suzuki modules properly. This is needed for
    ...
    try:
        if filename.startswith("<") and filename.endswith(">"):
            ...
            if (
                module.startswith("suzuki.modules")
                ...
                if isinstance(value, dict):
                    ...
                    try:
                        if (
                            ...
                            value,
                            (suzuki_tl.TelegramClient, CustomTelegramClient),
                        ):
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.suzuki_caller or client_id == record.suzuki_caller)
        ]
    ...
    async def _show_full_trace(
        ...
        item: SuzukiException,
    ):
        chunks = item.message + "\n\n<b>🪐 Full traceback:</b>\n" + item.full_stack

        chunks = list(utils.smart_split(*suzuki_tl.extensions.html.parse(chunks), 4096))
    ...
    def _gen_web_debug_button(self, item: SuzukiException) -> list:
        if not item.sysinfo:
            return []
        ...
        async with self._send_lock:
            ...
            self._exc_queue = {
                client_id: [
                    ...
                    for item in self.tg_buff
                    if isinstance(item[0], SuzukiException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_suzuki_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_suzuki_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        record.suzuki_caller = caller

        if record.levelno >= self.tg_level:
            if record.exc_info:
                exc = SuzukiException.from_exc_info(
                    *record.exc_info,
...
def init():
    ...
    logging.getLogger("suzuki_tl").setLevel(logging.WARNING)
    logging.getLogger("suzuki_pyro").setLevel(logging.WARNING)
