...
from suzuki_tl.errors import FloodWaitError, RPCError
from suzuki_tl.tl.types import Message
...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.suzuki_me.id
        self._cached_usernames = [
            (
                self._client.suzuki_me.username.lower()
                if self._client.suzuki_me.username
                else str(self._client.suzuki_me.id)
            )
        ]

        self._cached_usernames.extend(
            getattr(self._client.suzuki_me, "usernames", None) or []
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("suzuki-")
            and message.chat.title != "suzuki-logs"
        ):
            if not watcher:
