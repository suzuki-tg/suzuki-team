...
class UpdateNotifier(loader.Module):
    """Tracks latest Suzuki releases, and notifies you, if update is required"""
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "suzuki/update"},
                {"text": self.strings("ignore"), "data": "suzuki/ignore_upd"},
            ]
        )
        ...
        if self._pending not in {utils.get_git_hash(), self._notified}:
            m = await self.inline.bot.send_animation(
                ...
                caption=self.strings("update_required").format(
                    ...
                    '<a href="https://github.com/suzuki-team/Suzuki/compare/{}...{}">{}</a>'
                    .format(
    ...
    async def update(self, call: InlineCall):
        ...
        if call.data not in {"suzuki/update", "suzuki/ignore_upd"}:
            return

        if call.data == "suzuki/ignore_upd":
