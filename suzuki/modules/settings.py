...
class CoreMod(loader.Module):
    ...
    @loader.command()
    async def suzukicmd(self, message: Message):
        await utils.answer_file(
            message,
            "https://github.com/suzuki-team/assets/raw/master/suzuki_car_banner.mp4",
            self.strings("suzuki").format(
                (
                    utils.get_platform_emoji()
                    if self._client.suzuki_me.premium and CUSTOM_EMOJIS
                    else "🚗 <b>Suzuki userbot</b>"
                ),
                *version.__version__,
                utils.get_commit_url(),
                f"{suzuki.__version__} #{suzuki.tl.alltlobjects.LAYER}",
                (
                    "<emoji document_id=5377399247589088543>🔥</emoji>"
                    if self._client.pyro_proxy
                    else "<emoji document_id=5418308381586759720>📴</emoji>"
                ),
                f"{suzukipyro.__version__} #{suzukipyro.raw.all.layer}",
            )
            + (
                ""
