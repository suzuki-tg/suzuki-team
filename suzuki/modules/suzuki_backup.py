...
@loader.tds
class SuzukiBackupMod(loader.Module):
    """Handles database and modules backups"""

    strings = {"name": "SuzukiBackup"}

    async def client_ready(self):
        if not self.get("period"):
            await self.inline.bot.send_photo(
                self.tg_id,
                photo="https://github.com/suzuki-team/assets/raw/master/unit_alpha.png",
        ...
        self._backup_channel, _ = await utils.asset_channel(
            ...
            silent=True,
            archive=True,
            avatar="https://github.com/suzuki-team/assets/raw/master/suzuki-backups.png",
            _folder="suzuki",
        )
    ...
    async def handler(self):
        try:
            ...
            backup.name = (
                f"suzuki-db-backup-{datetime.datetime.now():%d-%m-%Y-%H-%M}.json"
            )
            ...
            logger.exception("SuzukiBackup failed")
    ...
    async def restoredb(self, message: Message):
        ...
        with contextlib.suppress(KeyError):
            decoded_text["suzuki.inline"].pop("bot_token")
