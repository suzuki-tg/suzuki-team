...
import suzukitl
from suzukitl.tl.functions.channels import JoinChannelRequest
from suzukitl.tl.functions.messages import (
    GetDialogFiltersRequest,
    UpdateDialogFilterRequest,
)
from suzukitl.tl.types import Message
from suzukitl.utils import get_display_name
...
@loader.tds
class SuzukiSettingsMod(loader.Module):
    """Advanced settings for Suzuki Userbot"""

    strings = {"name": "SuzukiSettings"}
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"suzuki-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "suzuki" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    @loader.command()
    async def uninstall_suzuki(self, message: Message):
    ...
    async def inline__setting(self, call: InlineCall, key: str, state: bool = False):
        if callable(key):
            key()
            suzukitl.extensions.html.CUSTOM_EMOJIS = not main.get_config_key(
    ...
    async def weburl(self, message: Message, force: bool = False):
        if "LAVHOST" in os.environ:
            form = await self.inline.form(
                ...
                reply_markup={
                    ...
                    "url": await main.suzuki.web.get_url(proxy_pass=False),
                },
                gif="https://t.me/suzuki_assets/28",
            )
            return
        ...
        if not main.suzuki.web:
            main.suzuki.web = core.Web(
                data_root=main.BASE_DIR,
                api_token=main.suzuki.api_token,
                proxy=main.suzuki.proxy,
                connection=main.suzuki.conn,
            )
            await main.suzuki.web.add_loader(self._client, self.allmodules, self._db)
            await main.suzuki.web.start_if_ready(
                len(self.allclients),
                main.suzuki.arguments.port,
                proxy_pass=main.suzuki.arguments.proxy_pass,
            )
        ...
        url = await main.suzuki.web.get_url(proxy_pass=True)

        await form.edit(
            self.strings("tunnel_opened"),
            reply_markup={"text": self.strings("web_btn"), "url": url},
            gif="https://t.me/suzuki_assets/48",
        )
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "flush_entity_cache":
                result = (
                    f"Dropped {len(self._client._suzuki_entity_cache)} cache records"
                )
                self._client._suzuki_entity_cache = {}
            elif method == "flush_fulluser_cache":
                result = (
                    f"Dropped {len(self._client._suzuki_fulluser_cache)} cache records"
                )
                self._client._suzuki_fulluser_cache = {}
            elif method == "flush_fullchannel_cache":
                result = (
                    f"Dropped {len(self._client._suzuki_fullchannel_cache)} cache"
                    " records"
                )
                self._client._suzuki_fullchannel_cache = {}
            elif method == "flush_perms_cache":
                result = f"Dropped {len(self._client._suzuki_perms_cache)} cache records"
                self._client._suzuki_perms_cache = {}
                ...
                result = (
                    f"Dropped {len(self._client._suzuki_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._suzuki_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._suzuki_fullchannel_cache)} fullchannel cache"
                    " records\nDropped"
                ...
                self._client._suzuki_fulluser_cache = {}
                self._client._suzuki_fullchannel_cache = {}
                self._client.suzuki_me = await self._client.get_me()
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._suzuki_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._suzuki_fulluser_cache)} records\nFullchannel"
                    " cache:"
                    f" {len(self._client._suzuki_fullchannel_cache)} records\nLoader"
