...
class Help(loader.Module):
    ...
    async def modhelp(self, message: Message, args: str):
        ...
        reply = "{} <b>{}</b>:".format(
            (
                DRAGON_EMOJI
                if is_dragon
                else "<emoji document_id=128683>🚗</emoji>"
            ),
            _name,
        )
        if module.__doc__:
            reply += (
                "<i>\n<emoji document_id=8506307431539835477>ℹ️</emoji> "
        ...
        if hasattr(module, "inline_handlers") and not is_dragon:
            for name, fun in module.inline_handlers.items():
                reply += (
                    "\n<emoji document_id=129302>🤖</emoji>"
                    " <code>{}</code> {}".format(
        ...
        for name, fun in commands.items():
            reply += (
                "\n<emoji document_id=11036>▫️</emoji>"
                " <code>{}{}</code>{} {}".format(
    ...
    async def support(self, message):
        if message.out:
            await self.request_join("@suzuki_talks", self.strings("request_join"))
