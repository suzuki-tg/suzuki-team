...
from suzukipyro import Client, errors, types
...
RED_CAR_EMOJI = "<emoji document_id=5375360100196163660>🚗</emoji>"
...
class ImportLock:
    # This is used to ensure, that dynamic suzuki import passes in
    # the right client. Whenever one of the clients attempts to install
    # suzuki-specific module, it must aqcuire the `import_lock` or wait
...
class SuzukiDb:
    ...
    def get(
        ...
        return self.db.get(f"suzuki.{module}", variable, default)

    def set(self, module: str, variable: str, value: JSONSerializable) -> bool:
        return self.db.set(f"suzuki.{module}", variable, value)

    def get_collection(self, module: str) -> typing.Dict[str, JSONSerializable]:
        return dict.get(self.db, f"suzuki.{module}", {})

    def remove(self, module: str, variable: str) -> JSONSerializable:
        if f"suzuki.{module}" not in self.db:
            return None

        return self.db[f"suzuki.{module}"].pop(variable, None)

    def close(self):
        pass


class SuzukiDbWrapper:
    def __init__(self, db: SuzukiDb):
        self.db = db
...
class SuzukiMisc:
    ...
    @property
    def prefix(self):
        return self.client.loader.get_prefix("suzuki")
...
class SuzukiScripts:
    def __init__(self, misc: SuzukiMisc):
    ...
    def format_small_module_help(self, module_name: str):
        ...
        help_text = (
            f"{RED_CAR_EMOJI}<b>Help for </b><code>{module_name}</code>\n\n<b>Commands"
            " list:</b>\n"
        )
...
class SuzukiCompat:
    def __init__(self, client: CustomTelegramClient):
        self.client = client
        self.db = SuzukiDbWrapper(SuzukiDb(client.loader.db))
        self.misc = SuzukiMisc(client)
        self.scripts = SuzukiScripts(self.misc)
        self.config = SuzukiConfig(client)


def patched_import(name: str, *args, **kwargs):
    caller = inspect.currentframe().f_back
    caller_name = caller.f_globals.get("__name__")
    if name.startswith("utils") and caller_name.startswith("suzuki"):
        if not import_lock.current_client:
            raise RuntimeError("Suzuki client not set")

        if name.split(".", maxsplit=1)[1] in {"db", "misc", "scripts", "config"}:
            return getattr(
                import_lock.current_client.suzuki_compat,
    ...
    if name.startswith("telethon"):
        return native_import("suzukitl" + name[8:], *args, **kwargs)

    if name.startswith("pyrogram"):
        return native_import("suzukipyro" + name[8:], *args, **kwargs)
...
def apply_compat(client: CustomTelegramClient):
    client.suzuki_compat = SuzukiCompat(client)