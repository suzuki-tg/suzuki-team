...
from suzukitl import TelegramClient
from suzukitl.errors.rpcerrorlist import TopicDeletedError
from suzukitl.hints import EntityLike
from suzukitl.network import MTProtoSender
from suzukitl.tl.functions.channels import GetFullChannelRequest
from suzukitl.tl.functions.users import GetFullUserRequest
from suzukitl.tl.tlobject import TLRequest
from suzukitl.tl.types import (
    ChannelFull,
    Message,
    Updates,
    UpdatesCombined,
...
from suzukitl.utils import is_list_like
...
class CustomTelegramClient(TelegramClient):
    def __init__(self, *args, **kwargs):
        ...
        self._suzuki_entity_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordEntity,
        ] = {}

        self._suzuki_perms_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordPerms,
        ] = {}

        self._suzuki_fullchannel_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullChannel,
        ] = {}

        self._suzuki_fulluser_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullUser,
        ] = {}
    ...
    @property
    def suzuki_entity_cache(self) -> typing.Dict[int, CacheRecordEntity]:
        return self._suzuki_entity_cache

    @property
    def suzuki_perms_cache(self) -> typing.Dict[int, CacheRecordPerms]:
        return self._suzuki_perms_cache

    @property
    def suzuki_fullchannel_cache(self) -> typing.Dict[int, CacheRecordFullChannel]:
        return self._suzuki_fullchannel_cache

    @property
    def suzuki_fulluser_cache(self) -> typing.Dict[int, CacheRecordFullUser]:
        return self._suzuki_fulluser_cache
    ...
    async def get_entity(
        ...
        _suzuki_client_id_logging_tag = copy.copy(self.tg_id)  # noqa: F841
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._suzuki_entity_cache
            and (
                not exp
                or self._suzuki_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
                "Using cached entity %s (%s)",
                entity,
                type(self._suzuki_entity_cache[hashable_entity].entity).__name__,
            )
            return copy.deepcopy(self._suzuki_entity_cache[hashable_entity].entity)
        ...
        if resolved_entity:
            ...
            self._suzuki_entity_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                self._suzuki_entity_cache[resolved_entity.id] = cache_record
            ...
            if getattr(resolved_entity, "username", None):
                ...
                self._suzuki_entity_cache[f"@{resolved_entity.username}"] = cache_record
                self._suzuki_entity_cache[resolved_entity.username] = cache_record
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._suzuki_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._suzuki_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
            logger.debug("Using cached perms %s (%s)", hashable_entity, hashable_user)
            return copy.deepcopy(
                self._suzuki_perms_cache[hashable_entity][hashable_user].perms
            )
        ...
        if resolved_perms:
            ...
            self._suzuki_perms_cache.setdefault(hashable_entity, {})[
                hashable_user
            ] = cache_record
            ...
            def save_user(key: typing.Union[str, int]):
                ...
                if getattr(user, "id", None):
                    self._suzuki_perms_cache.setdefault(key, {})[user.id] = cache_record

                if getattr(user, "username", None):
                    self._suzuki_perms_cache.setdefault(key, {})[
                        f"@{user.username}"
                    ] = cache_record
                    self._suzuki_perms_cache.setdefault(key, {})[
                        user.username
    ...
    async def get_fullchannel(
        ...
        if (
            not force
            and self._suzuki_fullchannel_cache.get(hashable_entity)
            and not self._suzuki_fullchannel_cache[hashable_entity].expired
            and self._suzuki_fullchannel_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._suzuki_fullchannel_cache[hashable_entity].full_channel

        result = await self(GetFullChannelRequest(channel=entity))
        self._suzuki_fullchannel_cache[hashable_entity] = CacheRecordFullChannel(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def get_fulluser(
        ...
        if (
            not force
            and self._suzuki_fulluser_cache.get(hashable_entity)
            and not self._suzuki_fulluser_cache[hashable_entity].expired
            and self._suzuki_fulluser_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._suzuki_fulluser_cache[hashable_entity].full_user

        result = await self(GetFullUserRequest(entity))
        self._suzuki_fulluser_cache[hashable_entity] = CacheRecordFullUser(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def _call(
        ...
        # list and you will be banned from Suzuki federation.
